...
GW_ADDR="dynamic-service-a23ff868ec8f.myservices.local"
...
(
    ...
    while true; do
        sleep 5
        ssh -L [$SRV_ADDR]:$MQTT_port:[$SRV_ADDR]:$MQTT_port -N -i /home/$USER/.ssh/id_ed25519 $USER@$GW_ADDR -p $SSH_port
    done
...
while true; do
    sleep 5
    mosquitto_pub -A ::1 -h $SRV_ADDR -p $MQTT_port -u $DEV_ADDR -t "iot/data" -m "Hello from IoT Device 1" --repeat 9999 --repeat-delay 5
done