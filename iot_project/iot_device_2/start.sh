...
while true; do
    ssh -L 1883:a23ff868ec8f.default.svc.cluster.local:1883 -N -i /root/.ssh/id_ed25519 -o StrictHostKeyChecking=no -o ExitOnForwardFailure=yes $USER@cmxsafe-gw.myservices.local -p 22 &
    ...
    while ps -p $SSH_PID > /dev/null; do
        sleep 5
        if mosquitto_pub -h 127.0.0.1 -p 1883 -t "iot/data" -m "Hello from IoT Device 2"; then
            FAIL=0
        else
            ((FAIL++))
